...
'
'     4. OLE Drag & Drop code based on vbAccelerator vbalTreeView.ctl
'
'        vbAccelerator TreeView Control
'        http://www.vbaccelerator.com/home/VB/Code/Controls/TreeView/TreeView_Control/VB6_TreeView_Full_Source_zip_vbalTreeView_ctl.asp
'----------------------------------------------------------------------------------------
'
' History:
'
...
'            - Added hWnd property (hWndTreeView).
...
'
'   * 1.3.0: - Added SetInsertionMark() method.
'            - Added SetHilitedNode() method. (*)
'              (*) This will interfere in NodeHilited() prop.
'                  Use NodeHilited() prop. for multil-select implementation.
'            - Added InsertMarkColor() prop.
'            - Added NodeLevel() prop.
'            - Added NodeTag() prop. (*)
'              (*) OLE Drag & Drop implementation can complicate copy/move of
'                  nodes due to oneness of keys. This lets store extra info for each
'                  node: You can generate 'auto-increased' keys and store meaningful
'                  information into node Tag.
'                  You can Set/Get manualy Tag through NodeTag() prop., or initialize
'                  it through AddNode() method (last opt. parameter).
'            - OLE Drag & Drop support:
'              > Added: OLEDragMode() prop.
'              > Added: OLEDropMode() prop.
'              > Added: OLEDragInsertStyle() prop.
'              > Added: OLEDragAutoExpand() prop.
'              > Added: OLEIsMyFormat() method.
'              > Added: OLEGetDragInfo() method.
'              > Added: OLEGetDropInfo() method.
'              > Added: OLEStartDrag() event.
'              > Added: OLEGiveFeedback() event.
'              > Added: OLEDragOver() event.
'              > Added: OLEDragDrop() event.
'              > Added: OLECompleteDrag() event.
'
'              More info:
'              http://www.devx.com/vb/Article/7864/1954?pf=true
'
'   * 1.3.1: - Forgotten:
'              - Added NodeExpanded() and NodeExpandedOnce() props.
'
'   * 1.3.2: - Added: CheckChildren() method (hereditary checking).
'            - Fixed: NodeDblClick not raised when DblClick on 'CheckBox'.
'                     Also node not expanded/collapsed.
'
'   * 1.3.3: - Added: Unicode support ?.
'----------------------------------------------------------------------------------------
'
' Known issues:
'
...
Private Type NMTREEVIEW
    hdr     As NMHDR
    Action  As Long
    itemOld As TVITEM_lp
    itemNew As TVITEM_lp
    ptDrag  As POINTAPI
End Type
...
Private Declare Function CreateFontIndirectW Lib "gdi32" (ByVal vv_lpLogFont As Long) As Long
...
Private Declare Function CreateWindowExW Lib "user32" (ByVal dwExStyle As Long, ByVal pv_lpClassName As Long, ByVal pv_lpWindowName As Long, ByVal dwStyle As Long, ByVal x As Long, ByVal y As Long, ByVal nWidth As Long, ByVal nHeight As Long, ByVal hwndParent As Long, ByVal hMenu As Long, ByVal hInstance As Long, lpParam As Any) As Long
...
Private Declare Sub MoveMemory Lib "kernel32" Alias "RtlMoveMemory" (ByVal pv_lpvDest As Long, ByVal pv_lpvSource As Long, ByVal cbCopy As Long)
Private Declare Sub ZeroMemory Lib "kernel32" Alias "RtlZeroMemory" (ByVal pv_lpvDest As Long, ByVal cbZeroed As Long)
...
Private m_eOLEDragMode        As tvOLEDragConstants            '# OLED&D
Private m_eOLEDropMode        As tvOLEDropConstants            '# OLED&D
Private m_eOLEDragInsertStyle As tvOLEDragInsertStyleConstants '# OLED&D
Private m_bOLEDragAutoExpand  As Boolean                       '# OLED&D
...
Private Const OLE_FORMAT_ID   As Long = &HFFFFB044 '# OLED&D (*)
Private Const T_EXPAND_DELAY  As Long = 1000       '# OLED&D

'(*) Same format ID used for vbAccelerator TreeView. Don't know if
'    something is behind of this particular ID. Anyway, used same one.
...
Private m_hNodeDrag              As Long    '# OLED&D
Private m_hNodeDrop              As Long    '# OLED&D
Private m_bNodeDropInsertAfter   As Boolean '# OLED&D
Private m_lExpandCounter         As Long    '# OLED&D
...
'
'                               *Note: Use m_uNodeData() to store extra node info.
'                                      Access to data through hNode-lParam -> array index.
...
Public Function InitializeImageList( _
    ...
    If (m_hTreeView) Then
        ...
        Call pvDestroyImageList
        m_hImageList = ImageList_Create(ImageWidth, ImageHeight, ILC_COLORDDB Or ILC_MASK, 0, 0)
...
'So, sort recursively using recursive call.
  ...
  Dim hNext As Long
    ...
    End If
End Sub
Public Function IsSiblingChecked(hNode As Long) As Boolean
  Dim hNext As Long
  IsSiblingChecked = False
        hNext = NodeFirstSibling(hNode)
        Do While hNext
            If NodeChecked(hNext) = True Then IsSiblingChecked = True: Exit Function
            If hNext = NodeLastSibling(hNext) Then Exit Function
            hNext = NodeNextSibling(hNext)
        Loop
End Function
Public Function IsChildCheckedAll(hNode As Long) As Boolean
  Dim hNext As Long
  If NodeChildren(hNode) = 0 Then
    If NodeChecked(hNode) = True Then IsChildCheckedAll = True Else IsChildCheckedAll = False
    Exit Function
  End If
  IsChildCheckedAll = True
        hNext = NodeChild(hNode)
        Do While hNext
            If NodeChecked(hNext) = False Then IsChildCheckedAll = False: Exit Function
            If hNext = NodeLastSibling(hNext) Then Exit Function
            hNext = NodeNextSibling(hNext)
        Loop
End Function
Public Sub CheckParent(ByVal hNode As Long, ByVal New_NodeChecked As Boolean)
  Dim hAtas As Long
    If (m_hTreeView) Then
        hAtas = SendMessageLong(m_hTreeView, TVM_GETNEXTITEM, TVGN_PARENT, hNode)
            If (New_NodeChecked) Then
                Call pvTVSetStateImage(hAtas, SII_CHECKED)
              Else
                If IsSiblingChecked(hNode) = True Then Exit Sub
                Call pvTVSetStateImage(hAtas, SII_UNCHECKED)
            End If
            If NodeLevel(hNode) = 0 Then Exit Sub
            Call CheckParent(hAtas, New_NodeChecked)
    End If
End Sub
Public Sub CheckChildren(ByVal hNode As Long, ByVal New_NodeChecked As Boolean)
  Dim hNext As Long
    If (m_hTreeView) Then
        ...
        Do While hNext
            If (New_NodeChecked) Then
                Call pvTVSetStateImage(hNext, SII_CHECKED)
              Else
                Call pvTVSetStateImage(hNext, SII_UNCHECKED)
            End If
...
Public Property Get NodeTag(ByVal hNode As Long) As String
    If (m_hTreeView) Then
        NodeTag = m_uNodeData(pvTVGetlParam(hNode)).sTag
    End If
End Property
Public Property Let NodeTag(ByVal hNode As Long, ByVal New_NodeTag As String)
    If (m_hTreeView) Then
        m_uNodeData(pvTVGetlParam(hNode)).sTag = New_NodeTag
    End If
End Property
...
Public Property Get NodeExpanded(ByVal hNode As Long) As Boolean
    If (m_hTreeView) Then
        NodeExpanded = pvTVGetState(hNode, [eStateExpanded])
    End If
End Property

Public Property Get NodeExpandedOnce(ByVal hNode As Long) As Boolean
    If (m_hTreeView) Then
        NodeExpandedOnce = pvTVGetState(hNode, [eStateExpandedOnce])
    End If
End Property
...
Public Property Let InsertMarkColor(ByVal New_InsertMarkColor As OLE_COLOR)
  ...
  Dim lColor As Long
    ...
    If (m_hTreeView) Then
        Call OleTranslateColor(New_InsertMarkColor, 0, lColor)
        Call SendMessageLong(m_hTreeView, TVM_SETINSERTMARKCOLOR, 0, lColor)
    End If
End Property
...
Public Property Set Font(ByVal New_Font As StdFont)
  ...
  Dim uLogFont As LOGFONTW
    ...
    If (m_hTreeView) Then
        ...
        m_hFont = CreateFontIndirectW(VarPtr(uLogFont))
        '###:Call SendMessageLong(m_hTreeView, WM_SETFONT, m_hFont, 1)
        Call SendMessageW(m_hTreeView, WM_SETFONT, m_hFont, 1)
    End If
End Property
...
Private Function pvCreateTreeView() As Boolean
  ...
  Dim lTVStyle As Long
    ...
    m_hTreeView = CreateWindowExW(lExStyle, StrPtr(WC_TREEVIEW), 0, lTVStyle, 0, 0, UserControl.ScaleWidth, UserControl.ScaleHeight, UserControl.hwnd, 0, App.hInstance, ByVal 0)
...
Private Sub pvStdFontToLogFont(oStdFont As StdFont, uLogFont As LOGFONTW)
  ...
  Dim lChar As Long
     ...
     With uLogFont
         ...
         '###:Next lChar
         Call ZeroMemory(VarPtr(.lfFaceName(0)), UBound(.lfFaceName()) + 1) '---memberi efek null-terminated-string.
         Call MoveMemory(VarPtr(.lfFaceName(0)), StrPtr(oStdFont.Name), Len(oStdFont.Name) * 2) 'oStdFont.Name
...
Private Function pvTVAddNode( _
  ...
  Dim hPrevious As Long
    ...
    pvTVAddNode = SendMessageW(m_hTreeView, TVM_INSERTITEMW, 0, VarPtr(uTVIS))
End Function

Private Function pvTVGetText(ByVal hNode As Long) As String
    
Dim uTVI   As TVITEM_lp
'###:Dim aBuf() As Byte
Dim szBuf   As String
    '###:ReDim aBuf(MAX_PATHW - 1)
    szBuf = String$(MAX_PATH, 0)
    With uTVI
        .hItem = hNode
        .mask = TVIF_TEXT
        '###.pszText = VarPtr(aBuf(0))
        .pszText = StrPtr(szBuf)
        .cchTextMax = MAX_PATH '---karena yg diminta jumlah karakter,bukan bytes.
    End With
    If (SendMessageW(m_hTreeView, TVM_GETITEMW, 0, VarPtr(uTVI))) Then
...
Private Sub pvTVSetText(ByVal hNode As Long, ByVal sText As String)
  ...
  Dim uTVI As TVITEM
    ...
    With uTVI
        ...
        .cchTextMax = Len(sText) + 1
    End With
    '###:Call SendMessage(m_hTreeView, TVM_SETITEMW, 0, uTVI)
    Call SendMessageW(m_hTreeView, TVM_SETITEMW, 0, VarPtr(uTVI))
End Sub
...
Private Sub pvTVHitTest(hNode As Long, lfHit As Long)
...
Private Function pvTVHasChildren(ByVal hNode As Long) As Boolean

    pvTVHasChildren = CBool(SendMessageLong(m_hTreeView, TVM_GETNEXTITEM, TVGN_CHILD, hNode))
End Function

Private Sub pvTVGetItemRect(ByVal hNode As Long, uRect As RECT2)
    
    Let uRect.X1 = hNode
    Call SendMessage(m_hTreeView, TVM_GETITEMRECT, 0, uRect)
End Sub
...
Private Sub pvSetIPAO()
  ...
  Dim uFrameInfo          As OLEINPLACEFRAMEINFO
    ...
    'Set pOleInPlaceSite = pOleObject.GetClientSite(pvClient)
...
Private Sub UserControl_OLEStartDrag(Data As DataObject, AllowedEffects As Long)
  ...
  Dim sData   As String
    ...
    If (m_hNodeDrag) Then
        ...
        sKey = m_uNodeData(pvTVGetlParam(m_hNodeDrag)).sKey
        sText = pvTVGetText(m_hNodeDrag)
        sData = "H:" & m_hTreeView & ";N:" & m_hNodeDrag
        aBuff() = sData
        Call Data.SetData(sText, vbCFText) 'Node text
        Call Data.SetData(aBuff(), OLE_FORMAT_ID)
    End If
...
Private Sub UserControl_OLEDragOver(Data As DataObject, Effect As Long, Button As Integer, Shift As Integer, x As Single, y As Single, State As Integer)
  ...
  Static bInsertAfter As Boolean
  ...
  Dim lfHit       As Long
    ...
    If (Not (OLEDropMode = vbOLEDropNone)) Then
        ...
        '-- Need to scroll ?
        Call GetClientRect(m_hTreeView, uRect) '(*)
        '(*) Not window rect.: take in account possible horizontal scroll bar.
        Call pvGetClientCursorPos(uPoint.x, uPoint.y)
        lItemHeight = SendMessageLong(m_hTreeView, TVM_GETITEMHEIGHT, 0, 0)
        
        Select Case True
            Case uPoint.y < lItemHeight
                Call SendMessageLong(m_hTreeView, WM_VSCROLL, SB_LINEUP, 0)
            Case uPoint.y > (uRect.Y2 - uRect.Y1) - lItemHeight And uPoint.y < (uRect.Y2 - uRect.Y1)
                If (NodeVisible(hNode)) Then '(*)
                    Call SendMessageLong(m_hTreeView, WM_VSCROLL, SB_LINEDOWN, 0)
                End If
        ...
        If (hNode) Then
            ...
            '-- Insert-mark ?
            If (m_eOLEDragInsertStyle = [disInsertMark]) Then
                Call pvGetClientCursorPos(uPoint.x, uPoint.y)
                Call pvTVGetItemRect(hNode, uRect)
                With uRect
                    m_bNodeDropInsertAfter = (uPoint.y > .Y1 + (.Y2 - .Y1) \ 2)
                End With
                If (bInsertAfter <> m_bNodeDropInsertAfter) Then
                    bInsertAfter = m_bNodeDropInsertAfter
              ...
              Else
                ...
                If (lfHit And (TVHT_ONITEMICON Or TVHT_ONITEMLABEL)) Then
                    If (m_bOLEDragAutoExpand) Then
                        If (pvTVHasChildren(hNode)) Then
                            If (pvTVGetState(hNode, [eStateExpanded]) = 0) Then
        ...
        '-- Finaly, ensure visible current drop-node (if any)
        Call SendMessageLong(m_hTreeView, TVM_ENSUREVISIBLE, 0, m_hNodeDrop)
...
Public Function OLEIsMyFormat(Data As DataObject) As Boolean
    
    OLEIsMyFormat = Data.GetFormat(OLE_FORMAT_ID)
End Function

Public Function OLEGetDragInfo(Data As DataObject, hWndTreeView As Long, hNodeDrag As Long) As Boolean

  Dim aBuff()     As Byte
  Dim sData       As String
  Dim lhWndPos    As Long
  Dim lhNodePos   As Long

    On Error Resume Next
    aBuff() = Data.GetData(OLE_FORMAT_ID)
    On Error GoTo 0
    sData = aBuff()

    If (Len(sData) > 6) Then
        lhWndPos = InStr(sData, "H:")

        If (lhWndPos = 1) Then
            lhNodePos = InStr(sData, ";N:")

            If (lhNodePos >= 4) Then
                
                '-- Try to interpret hWnd and hNode
                On Error Resume Next
                hWndTreeView = CLng(Mid$(sData, 3, lhNodePos - 3))
                hNodeDrag = CLng(pvStripNulls(Mid$(sData, lhNodePos + 3)))
                On Error GoTo 0

                '-- Success ?
                OLEGetDragInfo = (hWndTreeView <> 0 And hNodeDrag <> 0)
            End If
        End If
    End If
End Function

Public Sub OLEGetDropInfo(hNodeDrop As Long, InsertAfter As Boolean)
    
    hNodeDrop = m_hNodeDrop
    InsertAfter = IIf(m_eOLEDragInsertStyle = [disInsertMark], m_bNodeDropInsertAfter, True)
End Sub
Public Sub Matikan_Tree()
